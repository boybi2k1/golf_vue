...
const filteredTeeTimes = computed(() => {
  return teeTimes.value.filter((teeTime) => {
    const dateMatch =
      !filters.value.date || teeTime.date === filters.value.date;
    const courseMatch =
      !filters.value.golfCourseId ||
      teeTime.golfCourseId === filters.value.golfCourseId;
    const statusMatch =
      !filters.value.status || teeTime.status === filters.value.status;
    return dateMatch && courseMatch && statusMatch;
  });
});

const totalPages = computed(() => {
  return Math.max(1, Math.ceil(filteredTeeTimes.value.length / itemsPerPage));
});
...
const startItem = computed(() => {
  if (filteredTeeTimes.value.length === 0) return 0;
...
function loadTeeTimes() {
  ...
  const mockData = generateMockTeeTimes();
  teeTimes.value = mockData;
}
...
function generateMockTeeTimes() {
  ...
  const courses = ["GC001", "GC002", "GC003"];
  const mockData = [];

  // Generate tee times for each hour
  for (let hour = 6; hour <= 18; hour++) {
    const hourStr = hour.toString().padStart(2, "0");

    // Generate 2-3 tee times per hour for each course
    courses.forEach((course) => {
      const teeTimesCount = Math.floor(Math.random() * 2) + 2; // 2-3 tee times

      for (let i = 0; i < teeTimesCount; i++) {
        const minute = [0, 15, 30, 45][i % 4].toString().padStart(2, "0");
        ...
        let basePrice = 1500000; // 1.5M VND
        if (hour >= 9 && hour <= 15) basePrice = 2000000; // Peak hours
        if (course === "GC002") basePrice = basePrice * 0.7; // 9-hole course is cheaper
        ...
        const teeTime = {
          ...
          status: status,
          price: basePrice + Math.random() * 200000,
        };

        mockData.push(teeTime);
      }
    });
  }
